...
- Preserve Allure results or equivalent Allure run data even when tests fail. For Allure results, the directory basename should stay `allure-results`, especially when the data will be discovered by `allure run`.
...
For CI edits, validate syntax locally when tooling is available. If CI cannot be run locally, verify paths and commands against the existing workflow and clearly state that runtime validation requires a CI run.
